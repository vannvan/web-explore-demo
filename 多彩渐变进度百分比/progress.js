...
$.fn.extend({
  initData: function (initObj) {
    ...
    var innerEl =
      '<div class="m-percent-include" style="height: ' +
      initObj.height +
      'px;">' +
      '<div class = "m-no-percent" style="height: ' +
      initObj.height +
      'px;">' +
      '<div class = "m-bg-percent" style="height: ' +
      initObj.height +
      'px;"></div>' +
      '</div> </div> <span class="m-percent">' +
      initObj.percent +
      "% </span>";
    $(this).append(innerEl);
    var _this = $(this);
      ...
      setTimeout(function () {
        ...
        _this.find(".m-bg-percent").css({
          ...
          background:
            ...
            (initObj.bgStartColor ? initObj.bgStartColor : "#feea4d") +
            "," +
            (initObj.bgEndColor ? initObj.bgEndColor : "#ff3d00") +
            ")",
          left: 100 - initObj.percent + "%",
          transition: "left 2s ease-in-out",
        });
...
function mAutoPlay(el, percent, m_obj) {
  ...
  el.find(".m-bg-percent").css({
    ...
    background:
      "linear-gradient(to right," +
      (m_obj.bgStartColor ? m_obj.bgStartColor : "#feea4d") +
      "," +
      (m_obj.bgEndColor ? m_obj.bgEndColor : "#ff3d00") +
      ")",
    left: 100 - percent + "%",
  });
}
